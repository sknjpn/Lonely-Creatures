	...
	, drawObjectName(false)
{
	...
	assets.audio(U"かみさまのゆりかご.mp3").setLoop(true);
	//assets.audio(U"かみさまのゆりかご.mp3").play();
}
...
void	Game::update() {
	...
	Print << U"フルスクリーンにするにはF11キー、ヘルスバーを表示するにはHキーをおしてください。";
	Print << U"実行速度を変更するには、F1キー、もしくはF2キーを使ってください。" << U"実行速度 = " << playSpeed;
	Print << U"生物数:" << Creature::numEnabled << U" メモリ効率:" << int(Creature::numEnabled*100.0 / (double)field.creatures.size()) << U"%";
	Print << U"マテリアル数:" << Material::numEnabled << U" メモリ効率:" << int(Creature::numEnabled*100.0 / (double)field.creatures.size()) << U"%";
